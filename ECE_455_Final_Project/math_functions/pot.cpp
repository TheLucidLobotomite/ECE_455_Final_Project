...
double dr_global = 0.0;
    ...
    // use rp to find index into rho_global
    int idx = int(rp / dr_global);

    // safety clamp
    if (idx < 0) idx = 0;
    if (idx >= (int)rho_global.size()) idx = rho_global.size() - 1;

    double rho = rho_global[idx];  // ρ(r′)

    if (rp < r)
        return rho * (rp * rp) / r;  // inner region
    else
        return rho * rp;             // outer region
}
...
double r= step_size*sum; // multiply by step size  and return with runtime
 step_size =(2*M_PI-0)/steps; // calculate step size
 sum = 0;
...
int main() {
    ...
    for(int i=0; i<100; i++){
    sum1+=test_hydrogen_1s(h);
    //sum3+=test_gaussian(h);   
}
std::cout << "\n=== TEST 1: Hydrogen 1s orbital ===\n";
std::cout << "average speed up: "<<sum1/100<<"\n";
